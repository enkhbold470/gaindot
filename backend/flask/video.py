...
def squat_alignment(left_shoulder_x, squat_x_threshold):
    ...
    percentage_within_threshold_x = (
        within_threshold_count_x / len(left_shoulder_x)
    ) * 100
...
def squat_depth(shoulder, hip, knee, ankle):
    ...
    for i in range(len(shoulder)):
        ...
        cur_angle_hip_knee_ankle = angle_between_points(
            hip[i], knee[i], ankle[i]
        )  # Adjust the last parameter if needed

        # print(cur_angle_shoulder_hip_knee, cur_angle_hip_knee_ankle)

        if cur_angle_shoulder_hip_knee > 45:
            angle_shoulder_hip_knee = min(
                angle_shoulder_hip_knee, cur_angle_shoulder_hip_knee
            )
